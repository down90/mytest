...
<body>
   <%
      ...
      String saveDir=config.getServletContext().getRealPath(uploadDir);//*****외우기
      System.out.println("[1]upload path의 물리적 경로:"+ saveDir);
      //또는
      saveDir=application.getRealPath(uploadDir);//***************************외우기
      System.out.println("[2]upload path의 물리적 경로:"+ saveDir);
      
      //테스트 경로 : 실제배포는 위와같이 하나 매번테스트하기 번거로우니 테스트경로를 지정함 
      saveDir="D:\\down\\lecture\\workspace_list\\jsp_ws\\herbmall\\WebContent\\pds_upload";
      ...
      try{
         ...
         String id=request.getParameter("userid");//request. 하면 안됨!!!!! 주의하자
